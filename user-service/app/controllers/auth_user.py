...
def user_login(user_details: UserAuth, session: DB_SESSION):
    ...
    is_password_exist = verifyPassword(user_details.user_password, db_user.user_password)
    
    if not is_password_exist:
        raise HTTPException(status_code=404, detail="User does not exist with this password!")
